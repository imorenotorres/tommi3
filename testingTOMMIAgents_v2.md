...
- **Agent type & LLM provider** — shows the agent architecture (RAG, Metadata+RAG, Text2SQL, Oneshot) and whether the LLM is local or cloud. Click the "?" icon for details.
- **Agent tuning** — three clickable badges:
  - **LLM** — click to cycle through available models.
  - **Transparency** — click to switch between Crystal box, Grey box, and Black box.
...
### Phase 7 — LLM comparison (optional, 10-15 min)

| # | Test | Action | What to check |
|---|------|--------|---------------|
| 7.1 | Switch model | Click the LLM badge to switch to a different model | Does the badge update? |
| 7.2 | Repeat key queries | Re-ask 3-5 queries from Phase 2 with the new model | How does quality compare? |
| 7.3 | Note differences | Document where the alternative model is better or worse | Record in your notes |
