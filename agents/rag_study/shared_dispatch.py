"""
Shared dispatch logic for RAG Study variants V3 and V4.

Both variants use identical response paths — the only difference is
how the classification dict is produced:
  - V3: deterministic code classification (pattern matching)
  - V4: non-deterministic LLM classification

The classification dict has the format:
  {"category": str, "topic": str, "researcher": str, "university": str, "project": str}

Categories: meta, non_research, figure, project, researcher, glossary,
            gap, topic_search, university_papers, off_topic, followup, general
"""
...
def build_trace(classification: dict, action: str, production: str,
    ...
    """Build a decision trace (Perception -> Reasoning -> Action -> Production)."""
...
def dispatch(agent, classification: dict, user_message: str,
    ...
    agent: the Agent instance (provides access to config, data methods)
    classification: {"category": ..., "topic": ..., ...}
    user_message: original user query
    reasoning_label: label for the trace (e.g. "code patterns" or "LLM classification")
    """
    cat = classification.get("category", "general")
    ...
    if cat == "figure":
        ...
        trace = build_trace(classification, "Map link from regex extraction",
