# Health and Wellbeing Systems Agent -- Documentation

**Agent ID:** `health_wellbeing_sistems`
**Agent Name:** EH: Health and Wellbeing Systems
...
All data lives under `/agents/health_wellbeing_sistems/data/`.
...
| `docs/` | PDF, MD, TXT | Source documents. Includes the glossary (`Glossary_HealthWeillSystems_AI.md`) and paper PDFs. |
...
| `undefined_topics.jsonl` | JSONL | Logs queries that are on-topic (within Health and Wellbeing Systems scope) but have no glossary or database match. Each entry has `timestamp` and `query`. |
...
| # | Source | Example |
|---|--------|---------|
| 1 | Glossary concept names + abbreviations | "ai and healthcare", "wellbeing systems" |
...
| On-topic, undefined | Yellow | Query is in topical scope but no glossary/database match found | Topic is Health and Wellbeing Systems-related but not yet in the database |
...
When a query is within topical scope (`_is_in_topical_scope()` returns `True`) but no structured data, glossary entry, or RAG context is found, the query is logged to `data/undefined_topics.jsonl`. Each entry contains a UTC timestamp and the raw query text. This log helps content managers identify which Health and Wellbeing Systems concepts users are asking about but are not yet covered in the database or glossary.
...
1. Edit `data/docs/Glossary_HealthWeillSystems_AI.md`. Add a new entry using the format described in Section 2.7 below.
...
The `extra_scope_terms` array in `config.json` contains additional terms within the Health and Wellbeing Systems domain that may not appear in the glossary or paper topics. These are used by the two-axis banner system to distinguish on-topic queries from off-topic ones.

**When to add new terms:** When users ask about a Health and Wellbeing Systems concept and it gets a red "off-topic" banner instead of a yellow "on-topic, undefined" banner.
...
Edit the file `data/docs/Glossary_HealthWeillSystems_AI.md`. Each entry follows this format:
...
- "**Show a figure** of studies on the **topic** Wellbeing systems"
- "**List researchers** that have interest in Cancer detection"
- "**List the Health and AI subtopics** MOST studied in UNINOVIS"
- "**List any topics** related with Health and wellbeing systems that have not been studied"
- "**Show a figure** of the **collaborations** among the partners"
- "**Show a figure** of the collaborations in the **year** 2025"
- "List **research projects** on AI and Aging"
- "**Show a map** with the number of **research projects** per partner"
- "Tell me **an aging appropriate hot chocolate recipe**" (tests off-topic handling)
- "What are wellbeing systems?" (tests conceptual/glossary path)
...
**What it tells you:** Users are asking about a Health and Wellbeing Systems topic that the agent recognizes as in-scope (via the topical scope set) but cannot answer from the glossary or database.

**How to act on it:** Review the logged queries periodically. For frequently asked topics:
1. Add a glossary entry in `Glossary_HealthWeillSystems_AI.md`
...
|------------|----------------|
| "What are wellbeing systems?" | Green (glossary answer) or Yellow (if LLM diverges from glossary) |
| "List researchers from THUAS" | Green (structured data) |
| "Show a figure of Wellbeing systems studies" | Green (figure from database) |
| "List topics not studied in UNINOVIS" | Red (gap analysis / speculation) |
| "Tell me a cake recipe" | Red (off-topic / creative) |
| "What is digital phenotyping?" (if not in glossary but in extra_scope_terms) | Yellow (on-topic, undefined) |
| "Papers on AI and aging" | Green factual section + Yellow AI commentary |
...
This agent is a research assistant specialized in **UNINOVIS Health and Wellbeing Systems research**. It can answer questions about:
...
- Health and Wellbeing Systems concepts and definitions
...
| **On-topic, undefined** | Yellow | Your question is about a Health and Wellbeing Systems topic, but it is not yet covered in the database or glossary. The answer draws on the AI's general knowledge. Consider it informational, not authoritative. |
...
|----------|----------------|
| **Topic search** | "Papers on AI and aging", "Studies on cancer detection" |
...
| **Conceptual definitions** | "What are wellbeing systems?", "Define precision medicine", "What is digital health?" |
| **Research gap analysis** | "What topics have not been studied?", "List any missing research areas" |
| **Project search** | "List research projects on AI and Aging", "Projects funded in 2023" |
| **Collaboration maps** | "Show a figure of collaborations among the partners", "Collaborations in 2025" |
| **Interactive figures** | "Show a figure of studies on Wellbeing systems" (must include the word "figure" or "map") |
