# Data Collection for the Smart & Sustainable Environments Research Explorer

This folder contains the data files needed to complete the Smart & Sustainable Environments agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what Smart & Sustainable Environments means in the UNINOVIS context.

**Who**: Domain experts from the Smart & Sustainable Environments Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure architecture without smart technology, general environmental activism without AI or data)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Smart_Sustainable.md` — Domain glossary

A draft glossary with 25 entries covering smart cities, sustainable development, environmental AI, and energy efficiency. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `smart_sustainable_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `smart_sustainable_projects/`.
...
|---|---|---|
| `smart_sustainable_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `smart_sustainable_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's Smart & Sustainable Environments research.

### 3.1 Review papers — `smart_sustainable_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to Smart & Sustainable Environments. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `smart_sustainable_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Smart_Sustainable.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `smart_sustainable_papers.xlsx`, `smart_sustainable_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `smart_sustainable_papers.xlsx`, `smart_sustainable_projects/` |

---

## Project file format

For reference, each project in `smart_sustainable_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
