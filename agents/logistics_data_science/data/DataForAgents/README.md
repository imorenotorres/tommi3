# Data Collection for the Logistics & Data Science Research Explorer

This folder contains the data files needed to complete the Logistics & Data Science agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what Logistics & Data Science means in the UNINOVIS context.

**Who**: Domain experts from the Logistics & Data Science Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure theoretical mathematics without logistics application, general business management without data science)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Logistics_Data_Science.md` — Domain glossary

A draft glossary with 25 entries covering supply chain analytics, operations research, statistical methods, and smart logistics. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `logistics_data_science_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `logistics_data_science_projects/`.
...
|---|---|---|
| `logistics_data_science_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `logistics_data_science_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's Logistics & Data Science research.

### 3.1 Review papers — `logistics_data_science_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to Logistics & Data Science. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `logistics_data_science_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Logistics_Data_Science.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `logistics_data_science_papers.xlsx`, `logistics_data_science_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `logistics_data_science_papers.xlsx`, `logistics_data_science_projects/` |

---

## Project file format

For reference, each project in `logistics_data_science_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
