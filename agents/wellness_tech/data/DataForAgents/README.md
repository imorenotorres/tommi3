# Data Collection for the Wellness Technologies & Services Research Explorer

This folder contains the data files needed to complete the Wellness Technologies & Services agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what Wellness Technologies & Services means in the UNINOVIS context.

**Who**: Domain experts from the Wellness Technologies & Services Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure clinical medicine without technology, general sports science without digital component)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Wellness_Tech.md` — Domain glossary

A draft glossary with 25 entries covering wellness technology, digital services, health tech innovation, and preventive health. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `wellness_tech_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `wellness_tech_projects/`.
...
|---|---|---|
| `wellness_tech_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `wellness_tech_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's Wellness Technologies & Services research.

### 3.1 Review papers — `wellness_tech_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to Wellness Technologies & Services. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `wellness_tech_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Wellness_Tech.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `wellness_tech_papers.xlsx`, `wellness_tech_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `wellness_tech_papers.xlsx`, `wellness_tech_projects/` |

---

## Project file format

For reference, each project in `wellness_tech_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
