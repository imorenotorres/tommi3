# Data Collection for the AI & Robotics Research Explorer

This folder contains the data files needed to complete the AI & Robotics agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what AI & Robotics means in the UNINOVIS context.

**Who**: Domain experts from the AI & Robotics Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure mechanical engineering without AI, general electronics)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Robotics_AI.md` — Domain glossary

A draft glossary with 27 entries covering core robotics, AI/ML for robotics, systems and infrastructure, and application domains. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Currently 657 papers across 8 universities are pre-populated in `robotics_ai_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `robotics_ai_projects/`.
...
|---|---|---|
| `robotics_ai_papers.xlsx` | Publications (9 sheets: All + one per university) | Pre-populated |
| `robotics_ai_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's AI & Robotics research.

### 3.1 Review papers — `robotics_ai_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to AI & Robotics. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `robotics_ai_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Robotics_AI.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `robotics_ai_papers.xlsx`, `robotics_ai_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `robotics_ai_papers.xlsx`, `robotics_ai_projects/` |

---

## Project file format

For reference, each project in `robotics_ai_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
