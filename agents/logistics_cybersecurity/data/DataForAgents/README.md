# Data Collection for the Logistics & Cybersecurity Research Explorer

This folder contains the data files needed to complete the Logistics & Cybersecurity agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what Logistics & Cybersecurity means in the UNINOVIS context.

**Who**: Domain experts from the Logistics & Cybersecurity Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure hacking tutorials, general business management without cybersecurity, offensive security without research context)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Logistics_Cybersecurity.md` — Domain glossary

A draft glossary with 25 entries covering logistics systems, cybersecurity, secure supply chains, and digital infrastructure. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `logistics_cybersecurity_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `logistics_cybersecurity_projects/`.
...
|---|---|---|
| `logistics_cybersecurity_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `logistics_cybersecurity_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's Logistics & Cybersecurity research.

### 3.1 Review papers — `logistics_cybersecurity_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to Logistics & Cybersecurity. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `logistics_cybersecurity_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Logistics_Cybersecurity.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `logistics_cybersecurity_papers.xlsx`, `logistics_cybersecurity_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `logistics_cybersecurity_papers.xlsx`, `logistics_cybersecurity_projects/` |

---

## Project file format

For reference, each project in `logistics_cybersecurity_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
