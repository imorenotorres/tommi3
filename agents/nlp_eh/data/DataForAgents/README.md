# Data Collection for the AI & Language Research Explorer

This folder contains the data files needed to complete the AI & Language agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what AI & Language means in the UNINOVIS context.

**Who**: Domain experts from the AI & Language Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
### 1.3 `Glossary_NLP_AI_Language.md` — Domain glossary

A draft glossary with entries covering core NLP, AI/ML for language, multilingualism, and application domains. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `nlp_eh_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `nlp_eh_projects/`.
...
|---|---|---|
| `nlp_eh_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `nlp_eh_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's AI & Language research.

### 3.1 Review papers — `nlp_eh_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to AI & Language. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `nlp_eh_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_NLP_AI_Language.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `nlp_eh_papers.xlsx`, `nlp_eh_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `nlp_eh_papers.xlsx`, `nlp_eh_projects/` |

---

## Project file format

For reference, each project in `nlp_eh_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
