"""
RAG+Metadata Agent con ChromaDB
...
# Support both normal layout (../../web) and Docker layout (/app/web)
...
class Agent:
    def __init__(self, progress_callback=None):
        ...
        papers_json_path = os.path.join(os.path.dirname(__file__), "data", "papers.json")
...
GAP ANALYSIS — TOPICS NOT STUDIED:
When the user asks about topics that have NOT been studied, research gaps, or missing areas:
1. Look at the TOP RESEARCH TOPICS list provided in the metadata below — these are the topics that HAVE been studied.
2. Compare them against well-known {research_topic} subtopics (e.g., telemedicine, assistive robotics, digital therapeutics, AI-powered diagnostics, mental health apps, smart home for elderly, social prescribing, remote patient monitoring, etc.).
3. List subtopics that do NOT appear in the TOP RESEARCH TOPICS list.
4. NEVER list a paper as evidence of a topic that "has not been studied" — that is contradictory. If a paper exists on a topic, that topic HAS been studied.
...
- When listing researchers or papers:
  - If 15 or fewer results: provide the full list with details.
  - If more than 15 results: state the total count, show a summary (top 3-5 per university), and suggest the user refine the query. Use this exact format for the suggestion:
    "There are N results. To narrow down, you can add filters such as a **year** (e.g. 'papers on Ethics in 2025'), a **university** (e.g. 'TAMK papers on Ethics'), or a more **specific topic** (e.g. 'AI fairness in healthcare')."
  - For each university, use a compact format: "**ACRONYM** (N papers): Author1, Author2, Author3, ..."
- When listing papers, include the paper ID (e.g. W4377080381) so that PDF links can be generated automatically.
...
<body>
    ...
    def _pdf_link(self, paper_id: str) -> str:
        ...
        if not paper_id:
            return ""
        filename = f"{paper_id}.pdf"
        docs_path = os.path.join(os.path.dirname(__file__), "data", "docs", filename)
        if os.path.exists(docs_path):
            agent_id = self._config.get("agent_id", "health_tech")
    ...
    def _extract_topic(self, user_message: str) -> str:
        ...
        # Remove trailing university references and filler words
        universities = self._config.get("universities", {})
        for acronym in universities:
            topic = re.sub(r'\b' + re.escape(acronym) + r'\b.*$', '', topic, flags=re.IGNORECASE).strip()
        # Remove trailing filler like "per partner", "per university"
    ...
    def _extract_claims(response: str) -> list:
        ...
        # 2. Bold text in markdown (often paper titles or key terms)
        ...
        # 3. Author names: "Name Surname" patterns (capitalized words)
        ...
        # 5. University acronyms mentioned in context of results
        uni_refs = re.findall(r'\b(THUAS|UMA|TAMK|THWS|USPN|UDCLV|KK|UT)\b', response)
    ...
    @classmethod
    def _grounding_breakdown(cls, response: str, metadata_ctx: str, rag_ctx: str) -> dict:
        """Check claims against metadata (keyword) context and RAG context separately.

        Returns: {"metadata_pct": int, "database_pct": int, "llm_pct": int, "total_claims": int}
        - metadata_pct: % of claims grounded in structured metadata (keyword search)
        - database_pct: % of claims grounded in RAG chunks (not already in metadata)
        - llm_pct: % of claims not grounded in any source
        """
        claims = cls._extract_claims(response)

        if not claims:
            return {"metadata_pct": 100, "database_pct": 0, "llm_pct": 0, "total_claims": 0}
        ...
        for claim in claims:
            ...
            if metadata_lower and claim_lower in metadata_lower:
                metadata_count += 1
            elif rag_lower and claim_lower in rag_lower:
                database_count += 1
            else:
                # Fuzzy match: for multi-word claims (author names), try matching
                # the last word (surname) which is more likely to appear in PDFs
                words = claim_lower.split()
                fuzzy_matched = False
                if len(words) >= 2:
                    surname = words[-1]
                    if len(surname) > 3:
                        if metadata_lower and surname in metadata_lower:
                            metadata_count += 1
                            fuzzy_matched = True
                        elif rag_lower and surname in rag_lower:
                            database_count += 1
                            fuzzy_matched = True
    ...
    def _source_badge(source_type: str, breakdown: dict = None) -> str:
        ...
        Args:
            source_type: "Metadata", "Grounded", "Ungrounded", or None
            breakdown: {"metadata_pct": int, "database_pct": int, "llm_pct": int}
        """
        ...
        if breakdown and breakdown.get("total_claims", 0) > 0:
            parts = []
            if breakdown["metadata_pct"] > 0:
                parts.append(f"Metadata: {breakdown['metadata_pct']}%")
            if breakdown["database_pct"] > 0:
                parts.append(f"Database: {breakdown['database_pct']}%")
            if breakdown["llm_pct"] > 0:
                parts.append(f"LLM: {breakdown['llm_pct']}%")
            pct_str = f" ({' | '.join(parts)})"
            # Add clarification when there's a mix of sources
            llm_pct = breakdown.get("llm_pct", 0)
            if 0 < llm_pct < 100:
                grounded_sources = []
                if breakdown.get("metadata_pct", 0) > 0:
                    grounded_sources.append("structured metadata (keyword search)")
                if breakdown.get("database_pct", 0) > 0:
                    grounded_sources.append("document database (RAG)")
                sources_str = " and ".join(grounded_sources) if grounded_sources else "the database"
                note = f'<br><span style="font-weight:normal;font-size:0.8em;font-style:italic;">Factual claims are grounded in {sources_str}. Suggestions and interpretations may come from the LLM.</span>'
            else:
                note = ""
        else:
            if source_type == "Metadata":
                pct_str = " (Metadata: 100%)"
            else:
                pct_str = ""
            note = ""

        if source_type == "Metadata":
            # Green: grounded in structured metadata
            return (
        ...
        elif source_type == "Grounded":
            # Yellow: >=50% of claims grounded
            return (
                f'<div style="margin-bottom:10px;">'
    ...
    def chat(self, user_message: str, history: list = None) -> str:
        ...
        if is_followup:
            context = ""
            source_type = None  # No badge for follow-ups
        elif affiliation_ctx or uni_papers_ctx or topic_ctx or researcher_ctx or is_figure_request:
            context = ""
        ...
        # Gap analysis queries use LLM reasoning
        ...
        if source_type is None:
            ...
            badge = self._source_badge("Metadata", {"metadata_pct": 100, "database_pct": 0, "llm_pct": 0, "total_claims": 0})
        elif is_gap_analysis:
            badge = self._source_badge("Ungrounded", {"metadata_pct": 0, "database_pct": 0, "llm_pct": 100, "total_claims": 1})
        else:
            structured_ctx = " ".join(filter(None, [
                affiliation_ctx, uni_papers_ctx, topic_ctx, researcher_ctx, metadata_ctx
            ]))
            breakdown = self._grounding_breakdown(llm_content, structured_ctx, context)
            llm_pct = breakdown["llm_pct"]
            green_max = self._config.get("reliability_green_max_llm", 20)
            red_min = self._config.get("reliability_red_min_llm", 50)

            if llm_pct == 100 and self._is_not_found_response(llm_content):
                badge = self._source_badge("Metadata", breakdown)
            elif llm_pct <= green_max:
                badge = self._source_badge("Metadata", breakdown)
            elif llm_pct < red_min:
                badge = self._source_badge("Grounded", breakdown)
            else:
                badge = self._source_badge("Ungrounded", breakdown)
    ...
    async def chat_stream(self, user_message: str, history: list = None):
        ...
        # All badges deferred until we can compute grounding %; no badge for follow-ups
        # (badge emitted after full response is collected)
        ...
        if source_type is not None:
            if is_figure_request:
                yield ("badge", self._source_badge("Metadata", {"metadata_pct": 100, "database_pct": 0, "llm_pct": 0, "total_claims": 0}))
            elif is_gap_analysis:
                yield ("badge", self._source_badge("Ungrounded", {"metadata_pct": 0, "database_pct": 0, "llm_pct": 100, "total_claims": 1}))
            else:
                structured_ctx = " ".join(filter(None, [
                    affiliation_ctx, uni_papers_ctx, topic_ctx, researcher_ctx, metadata_ctx
                ]))
                breakdown = self._grounding_breakdown(full_response, structured_ctx, context)
                llm_pct = breakdown["llm_pct"]
                green_max = self._config.get("reliability_green_max_llm", 20)
                red_min = self._config.get("reliability_red_min_llm", 50)

                if llm_pct == 100 and self._is_not_found_response(full_response):
                    yield ("badge", self._source_badge("Metadata", breakdown))
                elif llm_pct <= green_max:
                    yield ("badge", self._source_badge("Metadata", breakdown))
                elif llm_pct < red_min:
                    yield ("badge", self._source_badge("Grounded", breakdown))
                else:
                    yield ("badge", self._source_badge("Ungrounded", breakdown))
