...
_ORACIONES_CORTAS = [
    ...
    ("Voy a la playa", "ˈboi a la 'pla.ʝa"),
    ("Quiero un vaso de agua", "ˈkie.ɾo un 'ba.so de 'a.gua"),
    ("El perro corre mucho", "el 'pe.ro 'ko.re 'mu.tʃo"),
    ("Dame la llave roja", "ˈda.me la 'ʝa.be 'ro.xa"),
    ("Llueve en la calle", "ˈʝue.be en la 'ka.ʝe"),
    ("Juan come arroz", "ˈxuan 'ko.me a.'roθ"),
    ("Pon el libro aquí", "ˈpon el 'li.bɾo a.'ki"),
    ("Hace mucho calor", "ˈa.θe 'mu.tʃo ka.'loɾ"),
    ("Cierra la ventana", "ˈθie.ra la ben.'ta.na"),
]
        ...
        if habla == 'errores':
            if nivel in (7, 8):
                ...
                for pt in palabras_trans:
                    ...
                    if pt.count('.') == 0 and len(pt.replace("ˈ", "")) <= 3:
...
def corregir_ejercicio_transcripcion(ejercicio: dict, respuestas_texto: str) -> tuple[str, list[str]]:
    ...
    for i, item in enumerate(ejercicio['items']):
        ...
        if not resp_fonol:
            ...
            avisos = _detectar_simbolos_aceptables(resp_fonol)
            if avisos:
                lineas.append(f"- ✅ Fonológica: /{resp_fonol}/ — **correcto**, pero en examen: {'; '.join(avisos)}")
            else:
        ...
        if tipo == 'fonetica':
            ...
            if solucion_fonet:
                ...
                if not resp_fonet:
                    ...
                    avisos = _detectar_simbolos_aceptables(resp_fonet)
                    if avisos:
                        lineas.append(f"- ✅ Fonética: [{resp_fonet}] — **correcto**, pero en examen: {'; '.join(avisos)}")
                    else:
...
# ── Símbolos aceptables (variantes no canónicas) ──
# Cada entrada: (variante_aceptable, símbolo_correcto, descripción_aviso)
_SIMBOLOS_ACEPTABLES = [
    # Marcas de acento
    ("'",      "ˈ", "usa ˈ (U+02C8) para el acento, no el apóstrofo '"),
    ("\u2018", "ˈ", "usa ˈ (U+02C8) para el acento, no la comilla simple '"),
    ("\u2019", "ˈ", "usa ˈ (U+02C8) para el acento, no la comilla simple '"),
    ("\u02B9", "ˈ", "usa ˈ (U+02C8) para el acento, no el modificador ʹ"),
    ("\u0060", "ˈ", "usa ˈ (U+02C8) para el acento, no el acento grave `"),
    ("\u00B4", "ˈ", "usa ˈ (U+02C8) para el acento, no el acento agudo ´"),
    # Vocales
    ("\u0251", "a", "usa a (U+0061) para la vocal abierta, no ɑ (U+0251)"),
    # Aproximantes sin diacrítico descendente
    ("β",  "β̞", "usa β̞ (con diacrítico descendente) para la aproximante bilabial"),
    ("ð",  "ð̞", "usa ð̞ (con diacrítico descendente) para la aproximante interdental"),
    ("ɣ",  "ɣ̞", "usa ɣ̞ (con diacrítico descendente) para la aproximante velar"),
]


_DIACRITICO_DESCENDENTE = "\u031E"  # ̞ combining lowered sign

# Aproximantes: solo reemplazar si NO van seguidas del diacrítico
_APROXIMANTES_ACEPTABLES = [
    ("β", "β̞"),
    ("ð", "ð̞"),
    ("ɣ", "ɣ̞"),
]

# Resto de sustituciones simples (sin complicaciones con diacríticos)
_SUSTITUCIONES_SIMPLES = [
    (v, c) for v, c, _d in _SIMBOLOS_ACEPTABLES
    if v not in ("β", "ð", "ɣ")
]


def _normalizar_transcripcion(t: str) -> str:
    """Normaliza una transcripción para comparación (acepta variantes)."""
    import re as _re
    t = t.strip()
    # Remove surrounding slashes/brackets
    t = t.strip('/').strip('[').strip(']').strip()
    # Sustituciones simples (acentos, vocales)
    for variante, correcto in _SUSTITUCIONES_SIMPLES:
        t = t.replace(variante, correcto)
    # Aproximantes: reemplazar β→β̞ solo cuando β NO va seguida de ̞
    for base, con_diacritico in _APROXIMANTES_ACEPTABLES:
        t = _re.sub(base + '(?!' + _DIACRITICO_DESCENDENTE + ')',
                    con_diacritico, t)
...
def _detectar_simbolos_aceptables(texto: str) -> list[str]:
    """Detecta variantes aceptables (no canónicas) usadas por el alumno.
    Devuelve lista de avisos (vacía si todo es canónico)."""
    import re as _re
    avisos = []
    avisos_vistos = set()
    for variante, _correcto, descripcion in _SIMBOLOS_ACEPTABLES:
        if variante in ("β", "ð", "ɣ"):
            # Solo avisar si la base aparece SIN el diacrítico descendente
            if _re.search(variante + '(?!' + _DIACRITICO_DESCENDENTE + ')', texto):
                if descripcion not in avisos_vistos:
                    avisos.append(descripcion)
                    avisos_vistos.add(descripcion)
        else:
            if variante in texto and descripcion not in avisos_vistos:
                avisos.append(descripcion)
                avisos_vistos.add(descripcion)
    return avisos
...
def _transcripciones_equivalentes(alumno: str, correcta: str) -> bool:
    """Compara dos transcripciones, tolerando variaciones aceptables."""
...
def _extraer_pares_error(alumno: str, correcta: str,
    ...
    for wi in range(min(len(palabras_a), len(palabras_c))):
        ...
        sils_a = pa.replace("ˈ", "").split('.')
        sils_c = pc.replace("ˈ", "").split('.')
