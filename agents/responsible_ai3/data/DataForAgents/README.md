# Data Collection for the Responsible AI Research Explorer

This folder contains the data files needed to complete the Responsible AI agent. The data collection follows three stages, each with a clear goal and responsible actors.

---

## Stage 1: Domain Definition (Excellence Hub experts)

**Goal**: Ensure the agent understands what Responsible AI means in the UNINOVIS context.

**Who**: Domain experts from the Responsible AI Excellence Hub (typically 1-2 people with broad knowledge of the field).
...
- Add topics that might seem related but are not (e.g., pure philosophy without AI, general law without algorithmic context)
- Remove examples that are actually in scope for your hub

### 1.3 `Glossary_Responsible_AI.md` — Domain glossary

A draft glossary with 25 entries covering explainability, fairness, governance, ethics, privacy, and trustworthiness. Each entry includes a definition, related concepts, and academic references.
...
- **Papers**: Collected from OpenAlex using the keywords from Stage 1. Pre-populated in `responsible_ai_papers.xlsx`.
- **Researchers**: Automatically extracted from paper author lists.
- **Projects**: Collected from CORDIS using keyword searches. One markdown file per project in `responsible_ai_projects/`.
...
|---|---|---|
| `responsible_ai_papers.xlsx` | Publications (9 sheets: All + one per university) | To be collected |
| `responsible_ai_projects/` | CORDIS project files (one `.md` per project) | To be collected |
...
**Who**: One contact person per university, ideally with knowledge of their institution's Responsible AI research.

### 3.1 Review papers — `responsible_ai_papers.xlsx`

Open your university's sheet and review each paper:

| Column | What to do |
|---|---|
| **Relevant?** (I) | Mark `Yes` if the paper is genuinely related to Responsible AI. Leave blank or `No` otherwise. Many papers may have been captured by broad keyword matching. |
...
### 3.2 Review projects — `responsible_ai_projects/`
...
|---|---|---|---|
| 1. Domain Definition | EH experts | Keywords, glossary, scope | `scope_keywords.txt`, `out_of_scope_examples.txt`, `Glossary_Responsible_AI.md` |
| 2. AI Data Collection | UMA-ICT team | Papers, researchers, projects from APIs | `responsible_ai_papers.xlsx`, `responsible_ai_projects/` |
| 3. Human Review | University contacts | Verify relevance, authors, affiliations | `responsible_ai_papers.xlsx`, `responsible_ai_projects/` |

---

## Project file format

For reference, each project in `responsible_ai_projects/` follows this format (one `.md` file per project, named `{GrantID}_{ShortName}.md`):
